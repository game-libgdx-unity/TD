...
public class ShieldCollisionBehaviour : MonoBehaviour
{
  ...
  public void ShieldCollisionEnter(CollisionInfo e)
  {
    if (e.Hit.transform!=null) {
      if (IsWaterInstance) {
        var go = ObjectPoolManager.Spawn(ExplosionOnHit) as GameObject;
        var t = go.transform;
        t.parent = transform;
        ...
        if (EffectOnHit!=null) {
          if (!CreateMechInstanceOnHit) {
            ...
            var effectInstance = ObjectPoolManager.Spawn(EffectOnHit) as GameObject;
        ...
        if (ExplosionOnHit!=null) {
          var inst2 = ObjectPoolManager.Spawn(ExplosionOnHit, e.Hit.point, new Quaternion()) as GameObject;
