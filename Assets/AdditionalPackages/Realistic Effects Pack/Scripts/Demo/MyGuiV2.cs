...
public class MyGuiV2 : MonoBehaviour
{
  ...
  {
    currentGo = ObjectPoolManager.Spawn(Prefabs[current], pos, Prefabs[current].transform.rotation) as GameObject;
  ...
  {
    currentGo = ObjectPoolManager.Spawn(Prefabs[current], GetInstancePosition(GuiStats[current]), Prefabs[current].transform.rotation) as GameObject;
    effectSettings = currentGo.GetComponent<EffectSettings>();
    effectSettings.Target = GetTargetObject(GuiStats[current]);
    if (isHomingMove) effectSettings.IsHomingMove = isHomingMove;
    prefabSpeed = effectSettings.MoveSpeed;
  ...
  {
    defaultBall = ObjectPoolManager.Spawn(Prefabs[1], transform.position, Prefabs[1].transform.rotation) as GameObject;
